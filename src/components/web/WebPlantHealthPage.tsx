...
export function WebPlantHealthPage({ user, plants }: WebPlantHealthPageProps) {
    ...
    try {
      ...
      const parsedResult: PlantHealthResult = {
        ...
        suggestions: suggestions.slice(0, 3).map((sug: any) => ({
          ...
          name:
            sug.plant_name ||
            sug.plant_details?.common_names?.[0] ||
            sug.plant_details?.scientific_name ||
            'Unknown Plant',
      ...
      if (suggestions.length > 0) {
        parsedResult.plantName =
          suggestions[0]?.plant_name ||
          suggestions[0]?.plant_details?.common_names?.[0] ||
      ...
      try {
        await saveDiagnosisHistory({
          ...
          plantName: parsedResult.plantName || selectedPlant?.name || 'Unknown Plant',
          scientificName: parsedResult.scientificName || selectedPlant?.species || '',
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white rounded-2xl p-6 border border-gray-200">
        <h3 className="text-xl text-gray-900 mb-2">Select Plant to Diagnose</h3>
        ...
        >
          <option value="">Scan without selecting a plant</option>
          {plants.map((p) => (
        ...
        {selectedPlant && (
          <div className="mt-3 bg-green-50 border border-green-200 rounded-xl p-3">
            <p className="text-sm text-green-900">
              Diagnosing selected plant: <span className="font-medium">{selectedPlant.name}</span>
            </p>
            <p className="text-xs text-green-700 mt-1">{selectedPlant.species}</p>
          </div>
        )}
      </div>
              ...
              {selectedPlant && (
                <div className="p-4 rounded-xl border bg-green-50 border-green-200">
                  ...
                  <p className="text-sm text-green-700 mt-1">
                    The uploaded image is being analyzed as this selected plant.
                  </p>
                </div>
              )}
      ...
      {history.length > 0 && (
        <div className="bg-white rounded-2xl p-6 border border-gray-200">
          ...
          <div className="space-y-4">
            {history.map((entry) => {
  const isOpen = selectedHistoryEntry?.id === entry.id;

  return (
    <div key={entry.id} className="space-y-3">
      <div
        onClick={() =>
          setSelectedHistoryEntry(isOpen ? null : entry)
        }
        className={`border rounded-2xl p-4 cursor-pointer transition-all ${
          isOpen
            ? 'border-green-300 bg-green-50/40'
            : 'border-gray-200 hover:shadow-md hover:border-green-200'
        }`}
      >
        <div className="flex gap-4">
          <img
      ...
      {isOpen && (
        <div className="bg-white rounded-2xl p-4 md:p-5 border border-gray-200 shadow-sm overflow-hidden">
          ...
          <div className="space-y-3 max-h-[420px] overflow-y-auto pr-2">
            {entry.diseases?.length > 0 ? (
              entry.diseases.map((disease: any, index: number) => (
                <div
                  key={index}
                  className="bg-gray-50 rounded-xl p-4 border border-gray-200 overflow-hidden"
                >
                  <div className="flex items-start justify-between gap-3 mb-2">
                    <h4 className="text-gray-900 flex-1 break-words">
                      {disease.name}
                    </h4>
                    <span className="px-2 py-1 bg-red-100 text-red-700 rounded-full text-xs whitespace-nowrap flex-shrink-0">
              ...
              <p className="text-green-700">No significant issues detected.</p>
            )}
          </div>
        </div>
      )}
